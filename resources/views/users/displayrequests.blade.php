...
<div class="site-blocks-cover inner-page-cover overlay" style="background-image: url('{{ asset('assets/images/hero_bg_2.jpg') }}');" data-aos="fade">
  <div class="container">
    <div class="row align-items-center justify-content-center text-center">
      <div class="col-md-10">
        <h1 class="mb-2">All property Requests</h1>
      </div>
    </div>
  </div>
</div>
  ...
  @if ($request->property)
    <div class="col-md-6 col-lg-4 mb-4">
      <div class="property-entry h-100">
        <a href="{{ route('single.prop', $request->property->id) }}" class="property-thumbnail d-block">
          <div class="offer-type-wrap">
            @if ($request->property->type == 'Buy')
              <span class="offer-type bg-success">{{ $request->property->type }}</span>
            @else
              <span class="offer-type bg-danger">{{ $request->property->type }}</span>
            @endif
          </div>
          <img src="{{ asset('assets/images/' . $request->property->image) }}" alt="Image" class="img-fluid rounded">
        </a>
        <div class="p-4 property-body">
          ...
          <ul class="property-specs-wrap mb-0">
            <li><span class="property-specs">Beds</span>
              <span class="property-specs-number">{{ $request->property->bed }} <sup>+</sup></span></li>
            <li><span class="property-specs">Baths</span>
              <span class="property-specs-number">{{ $request->property->baths }}</span></li>
            <li><span class="property-specs">SQ FT</span>
              <span class="property-specs-number">{{ $request->property->sq_ft }}</span></li>
          </ul>
        </div>
      </div>
    </div>
